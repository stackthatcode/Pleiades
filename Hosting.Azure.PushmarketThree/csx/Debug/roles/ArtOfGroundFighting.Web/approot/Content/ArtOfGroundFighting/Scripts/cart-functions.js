    ...
    },
    {
        ...
        ResponseFunction: function () {
            ngAjax.ModalGeneric(
                ...
                "Due to changes in inventory, items were removed from your cart.");
        }
    },
    {
        Id: "4",
        ResponseFunction: function () {
            ngAjax.ModalGeneric(
                "Items Added to Cart",
                "Due to changes in inventory, reduced quantities were added to your cart.");
        }
    },
    {
        Id: "5",
...
var AddQuantityListToCartItems = function(data) {
    for (var i = 0; i < data.Cart.CartItems.length; i++) {
        var item = data.Cart.CartItems[i];
        item.QuantityList = [];
        for (var quantity = 1; quantity <= item.Sku.Available; quantity++) {
            item.QuantityList.push(quantity);
        }
    }
